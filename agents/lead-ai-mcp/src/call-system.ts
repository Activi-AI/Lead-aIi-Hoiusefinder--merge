...
db.exec(\`
  ...
  -- Retry Rules: Wie oft und wann wiederholen?
...
function isInTimeWindow(trigger: any): boolean {
  const now = new Date();
  const day = ['So', 'Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa'][now.getDay()];
  
  if (!trigger.days.includes(day)) return false;
  
  const currentTime = now.toTimeString().slice(0, 5);
  return currentTime >= trigger.time_start && currentTime <= trigger.time_end;
}
...
export function registerCallSystemTools(server: McpServer) {
  ...
  server.tool(
    'trigger_create',
    {
      ...
      priority: z.number().optional().describe('PrioritÃ¤t (1-10)'),
    },
    async (params) => {
      ...
      return {
        content: [{
          type: 'text',
          text: \`â Trigger erstellt!
   ID: \${result.lastInsertRowid}
   Agent: \${params.agent_id}
   Name: \${params.name}
   Zeit: \${params.time_start || '09:00'} - \${params.time_end || '18:00'}
   Tage: \${params.days || 'Mo,Di,Mi,Do,Fr'}
   (erstellt von: \${getUsername()})\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async ({ agent_id }) => {
      ...
      let triggers;
      if (agent_id) {
        triggers = db.prepare('SELECT * FROM triggers WHERE agent_id = ?').all(agent_id);
      } else {
        triggers = db.prepare('SELECT * FROM triggers').all();
      }

      if (triggers.length === 0) {
        return { content: [{ type: 'text', text: 'ð Keine Trigger gefunden.' }] };
      }

      const text = triggers.map((t: any) => \`
ð Trigger #\${t.id}: \${t.name}
   Agent: \${t.agent_id}
   Status: \${t.enabled ? 'â Aktiv' : 'â Deaktiviert'}
   Zeit: \${t.time_start} - \${t.time_end}
   Tage: \${t.days}
   PrioritÃ¤t: \${t.priority}
\`).join('\\n');

      return { content: [{ type: 'text', text: \`=== TRIGGERS ===\${text}\` }] };
    }
  );
  ...
  server.tool(
    'trigger_toggle',
    {
      ...
      enabled: z.boolean().describe('Aktivieren (true) oder Deaktivieren (false)'),
    },
    async ({ trigger_id, enabled }) => {
      ...
      return {
        content: [{
          type: 'text',
          text: \`\${enabled ? 'â' : 'â'} Trigger #\${trigger_id} \${enabled ? 'aktiviert' : 'deaktiviert'}\`,
        }],
      };
    }
  );
  ...
  server.tool(
    'queue_add',
    {
      ...
      trigger_id: z.number().optional().describe('VerknÃ¼pfter Trigger'),
    },
    async (params) => {
      const authError = authCheck();
      if (authError) return authError;
      ...
      return {
        content: [{
          type: 'text',
          text: \`â Anruf zur Queue hinzugefÃ¼gt!
   Queue ID: \${result.lastInsertRowid}
   Telefon: \${params.phone}
   Kontakt: \${params.contact_name || 'Unbekannt'}
   PrioritÃ¤t: \${params.priority || 5}
   Geplant: \${params.scheduled_at || 'Sofort'}\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async ({ agent_id, status, limit }) => {
      ...
      if (queue.length === 0) {
        return { content: [{ type: 'text', text: 'ð Queue ist leer.' }] };
      }
...
\${statusIcons[q.status] || 'â'} Queue #\${q.id}
   Telefon: \${q.phone}
   Kontakt: \${q.contact_name || 'Unbekannt'}
   Status: \${q.status}
   Versuch: \${q.attempt}/\${q.max_attempts}
   Geplant: \${q.scheduled_at || 'Sofort'}
\`).join('\\n');

      return { content: [{ type: 'text', text: \`=== CALL QUEUE ===\${text}\` }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ agent_id }) => {
      ...
      // Hole nÃ¤chsten Anruf aus der Queue
      ...
      if (!next) {
        return { content: [{ type: 'text', text: 'ð Keine Anrufe in der Queue.' }] };
      }
      ...
      return {
        content: [{
          type: 'text',
          text: \`ð NÃ¤chster Anruf bereit:
   Queue ID: \${next.id}
   Telefon: \${next.phone}
   Kontakt: \${next.contact_name || 'Unbekannt'}
   Versuch: \${next.attempt + 1}/\${next.max_attempts}
   
   â¡ï¸ Nutze call_vonage_dial oder call_sipgate_dial um anzurufen!\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      ...
      const result = stmt.run(
        params.queue_id || null,
        params.agent_id,
        params.phone,
        params.contact_name || null,
        params.provider,
        params.call_uuid || null,
      ...
      return {
        content: [{
          type: 'text',
          text: \`â Call History Eintrag erstellt: #\${result.lastInsertRowid}\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      ...
      if (params.recording_url) {
        ...
        values.push(params.recording_url);
      }
      if (params.notes) {
        updates.push('notes = ?');
      ...
      if (updates.length === 0) {
        return { content: [{ type: 'text', text: 'â ï¸ Keine Updates angegeben.' }] };
      }
      ...
      return {
        content: [{
          type: 'text',
          text: \`â Call History #\${params.history_id} aktualisiert\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async ({ agent_id, phone, status, limit }) => {
      ...
      if (history.length === 0) {
        return { content: [{ type: 'text', text: 'ð Keine Anrufe in der History.' }] };
      }
...
\${statusIcons[h.status] || 'â'} Call #\${h.id} (\${h.provider})
   Telefon: \${h.phone}
   Kontakt: \${h.contact_name || 'Unbekannt'}
   Status: \${h.status}
   Dauer: \${h.duration_seconds || 0}s
   Zeit: \${h.started_at}
   \${h.result ? \`Ergebnis: \${h.result}\` : ''}
\`).join('\\n');

      return { content: [{ type: 'text', text: \`=== CALL HISTORY ===\${text}\` }] };
    }
  );
  ...
  server.tool(
    ...
    {
      agent_id: z.string().optional().describe('Filter nach Agent'),
      days: z.number().optional().describe('Tage zurÃ¼ck (default: 7)'),
    },
    async ({ agent_id, days }) => {
      const authError = authCheck();
      if (authError) return authError;
      ...
      return {
        content: [{
          type: 'text',
          text: \`=== CALL STATISTIKEN (letzte \${daysBack} Tage) ===

ð Ãbersicht:
   Gesamt: \${stats.total || 0} Anrufe
   Erfolgsrate: \${successRate}%

ð Status-Verteilung:
   â Abgeschlossen: \${stats.completed || 0}
   â Angenommen: \${stats.answered || 0}
   ðµ Nicht erreicht: \${stats.no_answer || 0}
   ð´ Besetzt: \${stats.busy || 0}
   â Fehlgeschlagen: \${stats.failed || 0}

â±ï¸ Dauer:
   Durchschnitt: \${Math.round(stats.avg_duration || 0)}s
   Gesamt: \${Math.round((stats.total_duration || 0) / 60)}min\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async (params) => {
      ...
      return {
        content: [{
          type: 'text',
          text: \`â Retry-Regel erstellt!
   ID: \${result.lastInsertRowid}
   Bei: \${params.call_result}
   Retry nach: \${params.retry_after_minutes || 60} Minuten
   Max Versuche: \${params.max_retries || 3}\`,
        }],
      };
    }
  );
  ...
  server.tool(
    ...
    async ({ agent_id }) => {
      ...
      let rules;
      if (agent_id) {
        rules = db.prepare('SELECT * FROM retry_rules WHERE agent_id = ?').all(agent_id);
      } else {
        rules = db.prepare('SELECT * FROM retry_rules').all();
      }

      if (rules.length === 0) {
        return { content: [{ type: 'text', text: 'ð Keine Retry-Regeln gefunden.' }] };
      }
...
ð Regel #\${r.id}
   Agent: \${r.agent_id}
   Bei: \${r.call_result}
   Retry nach: \${r.retry_after_minutes} Minuten
   Max Versuche: \${r.max_retries}
\`).join('\\n');

      return { content: [{ type: 'text', text: \`=== RETRY REGELN ===\${text}\` }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ queue_id, result, notes }) => {
      ...
      if (!queueItem) {
        return { content: [{ type: 'text', text: 'â Queue Item nicht gefunden.' }] };
      }

      // Hole Retry-Regel
      const retryRule = db.prepare(\`
        SELECT * FROM retry_rules 
        WHERE agent_id = ? AND call_result = ?
      ...
      db.prepare(\`
        ...
        WHERE id = ?
      \`).run(newStatus, nextRetry, queue_id);

      const message = newStatus === 'retry'
        ? \`ð Anruf wird wiederholt
   NÃ¤chster Versuch: \${nextRetry}
   Versuch \${queueItem.attempt}/\${retryRule?.max_retries || queueItem.max_attempts}\`
        : newStatus === 'completed'
        ? 'â Anruf erfolgreich abgeschlossen'
        : 'â Anruf endgÃ¼ltig fehlgeschlagen (max. Versuche erreicht)';

      return {
        content: [{
          type: 'text',
          text: \`Queue #\${queue_id}: \${result}
